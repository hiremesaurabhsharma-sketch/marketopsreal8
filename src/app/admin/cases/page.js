  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Incorrect Password! Not authorized to save.');
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Incorrect Password! Not authorized to delete.');
        setIsAuthenticated(false);
      } else {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <form onSubmit={handleLogin} className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100 max-w-md w-full">
          ...
          <div className="mb-6">
            ...
            <input 
              type="password" 
              required
              className="w-full border-slate-300 rounded-lg p-3 border focus:ring-2 focus:ring-indigo-500 outline-none" 
              value={password} 
